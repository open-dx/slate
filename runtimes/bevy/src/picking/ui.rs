...
use bevy::ecs::{prelude::*, query::QueryData};
use bevy::math::Vec2;
use bevy::render::prelude::*;
use bevy::transform::prelude::*;
use bevy::ui::{prelude::*, RelativeCursorPosition, UiStack};
use bevy::utils::hashbrown::HashMap;
use bevy::window::PrimaryWindow;

use bevy_mod_picking::picking_core::backend::prelude::*;
...
pub fn ui_picking(
    ...
    for ((camera, pointer), hovered_nodes) in hit_nodes.iter() {
        ...
        while let Some(node) = iter.fetch_next() {
            let Some(camera_entity) = node.target_camera.map(TargetCamera::entity).or(default_ui_camera.get()) else {
                tracing::debug!("No target camera found for UI Node!");
                continue;
            };
